psoptim <- function(FUN, n=100, max.loop=100, w=.9, c1=.2, c2=.2, xmin, xmax, vmax=c(4,4), seed=10, anim=TRUE)
{
  ...
  if((d == 2) && anim)
  {
    ...
    jpeg(paste("C:/Users/Konasz/Dysk Google/ETI/Magisterska/pso_easom_po_optym/0.jpg",sep=""))
  ...
  while(loop <= max.loop)
  {
    ...
    if((d==2) && anim)
    {
      ...
      jpeg(paste("C:/Users/Konasz/Dysk Google/ETI/Magisterska/pso_easom_po_optym/",loop,".jpg",sep=""))
  ...
  jpeg(paste("C:/Users/Konasz/Dysk Google/ETI/Magisterska/pso_easom_po_optym/wykres.jpg",sep=""))
  plot(g.best, type="o", col="darkgreen", pch=19, cex=.7, ylim=c(min(g.mean),max(g.best)), xlab="Iteracja", ylab="WartoϾ funkcji")
 ...
 # dev.off() ## do rysunkow do pdfa
  dev.off()
  res <<- list(sol = x.best.roju, val=g.best[loop-1], loop=loop-1)

    ...
    for(i in 1:powt) #ilosc powtorzen tej samej funkcji tym samym algorytmem i ta sama liczebnoscia
    {
      ...
      if(i==powt && j==5)
      {
        ...
        outDEoptim = DEoptim(fn=funkcje[[k]], lower, upper,
                              DEoptim.control(itermax = 100,storepopfrom = 1, storepopfreq = 1, NP=il.robali[j],trace=FALSE))
        #czas
        ...
        outDEoptim = DEoptim(fn=funkcje[[k]], lower, upper,
                              DEoptim.control(VTR = -min[k],itermax = 100,storepopfrom = 1, storepopfreq = 5, NP=il.robali[j],trace=FALSE))
        #czas
        wynikczasu= proc.time() - ptm
